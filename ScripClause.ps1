...
function Get-CMLogDirectory {
    [CmdletBinding()]param()
    try {$LogDir = (Get-ItemProperty -Path registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\CCM\Logging\@Global\ -Name LogDirectory -ErrorAction Stop).LogDirectory}
    catch {$LogDir = $null}
...
function Test-IsRunningAsSystem {
    [CmdletBinding()]param()
    $RunningAsSystem = $env:username -eq "${env:computername}$"
...
function Get-PMPLogPath {
    ...
    if (Test-IsRunningAsSystem) {
        ...
        if ($null -ne $CMLogDir -and (Test-Path -Path $CMLogDir)) {$LogPath = $CMLogDir}
        else {
            if (Test-Path -Path "$env:programdata\PatchMyPCIntuneLogs") {$LogPath = "$env:programdata\PatchMyPCIntuneLogs"}
            else {
                Try {
